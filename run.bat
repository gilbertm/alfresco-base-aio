...
:build_share
    ...
    docker compose -f "%COMPOSE_FILE_PATH%" rm -f aio-share
	call %MVN_EXEC% clean package -pl aio-share,aio-share-docker
EXIT /B 0
:build_acs
    docker compose -f "%COMPOSE_FILE_PATH%" kill aio-acs
    docker compose -f "%COMPOSE_FILE_PATH%" rm -f aio-acs
	call %MVN_EXEC% clean package -pl aio-integration-tests,aio-platform,aio-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl aio-platform,aio-integration-tests,aio-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl aio-platform,aio-integration-tests
EXIT /B 0
:purge
